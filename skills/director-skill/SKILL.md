...
黄油脆丝棒还必须执行 [references/butter-crisp-stick-pixel-qa.md](references/butter-crisp-stick-pixel-qa.md) 的逐对象原尺寸审核，并运行 `scripts/audit_butter_crisp_qa.py --manifest <qa-manifest.json>`。联系表只负责定位镜头和局部，不能承担批准；任何可见产品或外盒缺少独立实例记录时整镜不得批准。包装正面在成片中达到可读尺寸时，品牌、品名和净含量必须由批准包装参考像素做确定性透视贴合并保护遮挡关系，禁止让生成模型重写中文字形或版式。
